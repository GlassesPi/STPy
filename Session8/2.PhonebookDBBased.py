...
def search():
    ...
    conn = sqlite3.connect('my_db.db')
    c = conn.cursor()
    c.execute("SELECT * FROM phone WHERE name LIKE '{}' or phone like '{}'".format(search_entry.get(), search_entry.get()))
...
Button(root, text="Search", command=search).grid(row=7, column=1)
